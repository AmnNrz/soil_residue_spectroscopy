...
x_breaks_list <- list(
  NDTI   = c(-0.2, -0.1, 0, 0.1, 0.2),
  CAI    = c(2, 4, 6, 8, 10, 12),
  SINDRI = c(-4, -2, 0, 2, 4, 6, 8, 10, 12)
)

x_limits_list <- list(
  NDTI   = range(x_breaks_list$NDTI),
  CAI    = range(x_breaks_list$CAI),
  SINDRI = range(x_breaks_list$SINDRI)
)
...
for (sl in unique(df_to_plot$soil)) {
  ...
  for (idx_ in levels(df_to_plot$index_name)) {
    ...
    y_axis_theme <- if (idx_ == levels(df_to_plot$index_name)[1]) {
    ...
    p <- ggplot(filtered, aes(x = index, y = Fraction_Residue_Cover, color = as.factor(crop))) +
      ...
      scale_x_continuous(
        breaks = x_breaks_list[[idx_]],
        limits = x_limits_list[[idx_]],
        expand = c(0, 0)
      ) +
      labs(x = idx_, y = if (idx_ == "NDTI") "Fraction Residue Cover (%)" else NULL) +
      theme_minimal() +
      theme(aspect.ratio = 1, 
            legend.position = c(0.42, -0.02),
  ...
  ggsave(paste0(path_to_plots, 'fr_index_dry_fits/fresh/agu/', sl, "_combined.png"),
         final_plot, width = 16, height = 5, dpi = 300)
}
...
# 4) Keep labels only for boundaries that define the red uncertain spans
uncertain_bounds <- uncertain %>%
  select(index_name, boundary_index = start) %>%
  bind_rows(
    uncertain %>%
      select(index_name, boundary_index = end)
  ) %>%
  distinct()
...
df_labels <- df_intervals %>%
  ...
  semi_join(uncertain_bounds, by = c("index_name", "boundary_index")) %>%   # keep only red-bar bounds
  left_join(offset_lookup, by = "index_name") %>%
  group_by(index_name, crop) %>%
  mutate(
    span   = max(df_intervals$xmax[df_intervals$index_name == first(index_name)]) -
      min(df_intervals$xmin[df_intervals$index_name == first(index_name)]),
...
p <- ggplot() +
  ...
  facetted_pos_scales(
    x = list(
      index_name == "NDTI" ~ scale_x_continuous(
        breaks = x_breaks_list$NDTI,
        limits = x_limits_list$NDTI,
        expand = c(0, 0)
      ),
      index_name == "CAI" ~ scale_x_continuous(
        breaks = x_breaks_list$CAI,
        limits = x_limits_list$CAI,
        expand = c(0, 0)
      ),
      index_name == "SINDRI" ~ scale_x_continuous(
        breaks = x_breaks_list$SINDRI,
        limits = x_limits_list$SINDRI,
        expand = c(0, 0)
      )
    )
  ) +
...
global_crop_legend <- ggplot(df_to_plot,
                             aes(x = 1, y = 1, color = as.factor(crop), shape = as.factor(crop))) +
  geom_point(size = 3) +
  scale_color_manual(name = NULL , values = custom_colors, labels = levels(as.factor(df_to_plot$crop))) +
  scale_shape_manual(name = NULL, values = custom_shapes, labels = levels(as.factor(df_to_plot$crop))) +
  theme_minimal() +
  theme(
    legend.position = "right",  # Place the legend on the right
    legend.title = element_text(size = base_size * 1, color="black"),
...
for (crp in unique(df_to_plot$crop)) {
  ...
  ggsave(paste0(path_to_plots, 'fr_index_dry_fits/soil/agu/', crp, "_", "combined", '.png'), 
         final_plot, width = 15, height = 4.5, dpi = 300)
...
# Make a consistent soil factor (same levels everywhere)
soil_levels <- sort(unique(df_intervals$soil))  # or use your preferred order

df_intervals2 <- df_intervals %>%
  mutate(soil_f = factor(soil, levels = soil_levels),
         y_pos  = as.numeric(soil_f) + y_offset)

df_labels2 <- df_labels %>%
  mutate(soil_f = factor(soil, levels = soil_levels),
         y_pos  = as.numeric(soil_f) + y_offset)
...
p <- ggplot() +
  ...
  # geom_text(
  #   data = df_labels2,
  #   aes(x = x_plot, y = y_pos, label = lab),
  #   inherit.aes = FALSE,
  ...
  scale_y_continuous(
    breaks = seq_along(soil_levels) + y_offset,
    labels = soil_levels
  ) +
  scale_fill_manual(
    values = c(
...
fit_on_dry_wheat_Athena <- function(data) {
  # Find the minimum RWC in this subset
  filtered_data <- data %>%
    filter((crop == "Wheat Norwest Duet") & soil == "Athena")
  
  # Fit linear model to the data with minimum RWC
  model <- lm(Fraction_Residue_Cover ~ index, data = filtered_data)
  ...
  # # Calculate predictions for the minimum RWC data only
  # predictions_min_rwc <- predict(model, newdata = filtered_data)
...
# df_interpolated <- df %>%
#   group_by(index_name, mix) %>%  # Group by index_name, RWC, and mix
#   reframe(
...
px_side <- 800        # <- panel side length you want (pixels)
dpi_out <- 300
in_side <- px_side / dpi_out   # inches needed for that many pixels

index_names <- c("NDTI", "CAI", "SINDRI")
plot_list <- vector("list", length(index_names))
...
for (i in seq_along(index_names)) {
  ...
  p <- ggplot(sub_df, aes(x = index, y = fr_ab, color = crop)) +
    ...
    labs(x = idx, y = "Mean Absolute Bias", color = "Crop Types") +
    scale_y_continuous(
      limits = c(0, 1),
      breaks = seq(0, 1, by = 0.2),
      expand = c(0, 0)
    ...
    theme(
      ...
      # <-- this is the key: force each panel to be square
      aspect.ratio = 1
    )
...
final_plot <- wrap_plots(plot_list, nrow = 1, guides = "collect") &
  theme(legend.position = "bottom")

print(final_plot)
# Save: width = (n panels)*panel_side_in + some extra room for spacing/legend
ggsave(
  filename = paste0(path_to_plots, "mab/by_crops.png"),
  plot     = final_plot,
  width    = length(index_names) * in_side + 1.0,  # tweak +1.0 if legend cramped
  height   = in_side + 1.2,                         # tweak for legend space
  dpi      = dpi_out
)
...
panel_px <- 800
dpi_out  <- 300
panel_in <- panel_px / dpi_out

x_breaks_list <- list(
  NDTI   = c(-0.2, -0.1, 0, 0.1, 0.2),
  CAI    = c(2, 4, 6, 8, 10, 12),
  SINDRI = c(-2, 0, 2, 4, 6, 8, 10, 12)
)
...
for (i in seq_along(index_names)) {
  ...
  p <- ggplot(sub_df, aes(x = index, y = fr_ab, color = crop)) +
    ...
    scale_x_continuous(
      breaks = x_breaks_list[[idx]],
      limits = x_limits_list[[idx]],
      expand = c(0, 0)) +
    common_theme
  ...
  if (i == 1) {
    p <- p + theme(plot.margin = margin(5.5, panel_gap, 5.5, 5.5))
  }
  if (i == 2) {
    p <- p + theme(plot.margin = margin(5.5, panel_gap, 5.5, panel_gap))
  }
  if (i == 3) {
    p <- p + theme(plot.margin = margin(5.5, 5.5, 5.5, panel_gap))
  }
  
  plot_list[[i]] <- p
}

final_plot_crop <- wrap_plots(plot_list, nrow = 1, guides = "collect")

ggsave(
  filename = paste0(path_to_plots, "mab/by_crops.png"),
  plot     = final_plot_crop,
  width    = 3 * panel_in + 3,
  height   = panel_in + 0.4,
  dpi      = dpi_out
)
...
# ---- choose your target square panel size (pixels) ----
panel_px <- 800   # <- each subplot panel will be ~panel_px x panel_px pixels
dpi_out  <- 300
panel_in <- panel_px / dpi_out  # inches per panel side
...
for (i in seq_along(index_names)) {
  ...
  p <- ggplot(sub_df, aes(x = index, y = fr_ab, color = soil)) +
    ...
    theme(
      ...
      # ---- KEY: force the plotting panel to be square ----
      aspect.ratio = 1
    )
...
# ---- Save with size based on panel pixel target ----
# Start with panel geometry (3 panels in a row), then add a bit for spacing + legend.
ggsave(
  filename = paste0(path_to_plots, "mab/by_soils.png"),
  plot     = final_plot,
  width    = 3 * panel_in + 1.0,  # +1.0 inch buffer for gaps/margins
  height   = panel_in + 1.4,      # +1.4 inch buffer for legend at bottom
  dpi      = dpi_out
)
...
for (i in seq_along(index_names)) {
  ...
  p <- ggplot(sub_df, aes(x = index, y = fr_ab, color = soil)) +
    ...
    labs(x = idx, y = "Mean Absolute Bias", color = "Soil Type") +
    common_theme
...
final_plot_soil <- wrap_plots(plot_list, nrow = 1, guides = "collect")

ggsave(
  filename = paste0(path_to_plots, "mab/by_soils.png"),
  plot     = final_plot_soil,
  width    = 3 * panel_in + 3,
  height   = panel_in + 0.4,
  dpi      = dpi_out
)
